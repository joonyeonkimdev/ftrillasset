...
def bollingerband1(company_name,start_date=None,end_date=None):
   ...
   df= mk.get_daily_price(company_name,start_date=None,end_date=None)#타입에러
   ...
   for i in range(len(df.CLOSE)):
        if df.PB.values[i]>0.8 and df.MFI10.values[i]>80:#추세추종 매수
            ...
            df[19:]
   plt.savefig('Bollingerband1.png')        
   plt.legend(loc='best')
   plt.title('볼린저밴드')
...
def bollingerband2(company_name,start_date=None,end_date=None):
          ...
          Analyzer.MarketDB()
          mk = Analyzer.MarketDB()
          dr= mk.get_daily_price(company_name,start_date=None,end_date=None)#타입에러
          ...
          dr['II']=(2*dr['CLOSE']-dr['HIGH']-dr['LOW'])/(dr['HIGH']-dr['LOW'])*dr['VOLUME']
          ...
          plt.savefig('Bollingerband2.png')       
          plt.legend(loc='best')
...
def signallist(company_name,start_date=None,end_date=None):
    ...
    dic={1:df.CLOSE.values[i],2:df.CLOSE.values[i],3:dr.CLOSE.values[i],4:dr.CLOSE.values[i]}
